...
- `<测试文件路径> -rvtest` 运行对应测试文件的差分测试
  ...
  - 在上述命令后继续增加参数：
    ...
    - `-printpc` 每隔固定周期输出一次 PC、物理 PC 和指令
    - `-delay` 行为不一致时将继续运行一段时间再停止
    - `-starttrace <开始记录波形的时刻>` 从指定仿真时刻开始记录波形
  - 评测功能，下述信号互斥，只能选择一个：
...
- `-os` 运行操作系统测试（目前支持Linux）
  - 在上述命令后继续增加参数：
    ...
    - `-tracepcseq` 匹配内置 PC 序列后自动开始记录波形，默认关闭
    - `-emu` 使用模拟器进行测试

  - OS 模式下如果长时间没有指令提交，仿真器会在达到 stuck 判定阈值的一半时自动打开 `trace.fst`，用于保留 stuck 前后的波形；不会再生成 `stuck_trace.txt`。
